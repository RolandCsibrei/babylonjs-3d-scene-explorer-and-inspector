...
export class HtmlToTextureScene extends BaseScene {
  ...
  public setTexture1(textureBuffer: ArrayBufferView | undefined) {
    if (textureBuffer) {
      ...
      console.warn('TextureBuffer1 disconnected')
    }
  }
  ...
  public setTexture2(textureBuffer: ArrayBufferView | undefined) {
    if (textureBuffer) {
      ...
      console.warn('TextureBuffer2 disconnected')
    }
  }
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this._scene.onPointerPick = (e, pickInfo) => {
      this._getHtmlElementFromPickInfo(pickInfo, true) // simulate click
    }
  }
  ...
  private _getHtmlElementFromPickInfo(pickInfo: PickingInfo, simulateClick = false) {
    ...
    if (cords) {
      const offsetX = pickInfo.pickedMesh?.name === 'htmlPlane2' ? WIDTH - PANEL_WIDTH : 0

      const htmlX = offsetX + cords.x * PANEL_WIDTH
